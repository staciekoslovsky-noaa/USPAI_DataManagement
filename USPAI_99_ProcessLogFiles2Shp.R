...
log_folder <- 
  ...
  '\\\\akc0ss-n086\\NMML_USPAI_Imagery\\shemya_2023\\FlightLogs_FromSam\\'
log_file_type <-
  #'log' #fl02 to fl04
  'csv' #fl05, fl10, fl11
log_file <- 
  #'40Piccolo 5274 Tue Sep 12 11-44-56 2023.log' #fl02 to fl04
  #'41Piccolo 5274 Fri Sep 15 18-55-38 2023.csv' #fl05
  #'42Piccolo 5274 Tue Sep 19 13-08-30 2023.csv' #fl10
  '43Piccolo 5274 Tue Sep 19 13-08-30 2023.csv' #fl11
year_filter <- 2023
month_filter <- 9
day_filter <- 
  #list(12, 13) # log only
...
export_folder <- 
  ...
  '\\\\akc0ss-n086\\NMML_USPAI_Imagery\\shemya_2023\\FlightLogs_ToShp\\'
export_file <- 
  #'flightLog_20230912_fl02tofl04.shp'
  #'flightLog_20230915_fl05.shp'
  #'flightLog_20230919_fl10.shp'
  'flightLog_20230919_fl11.shp'
...
if(log_file_type == 'csv') {
  log <- read.table(log_file, skip = 1, sep = ',')
  colnames(log) <- tolower(c('Time_sec',	'Hours', 'Minutes', 'Seconds',
                             'GPS_kts', 'TAS_kts', 'IAS_kts', 'BaroAlt_ft', 'AGL_ft', 'Height_ft', 'Xaccel_gs', 'Yaccel_gs', 'Zaccel_gs',
                             'Vdown_ftps', 'Pitch_deg', 'Roll_deg', 'Lrudder_deg', 'Rrudder_deg', 'Flaps_deg', 'L_RPM_rpm', 'Throttle', 'Fuellevel_mV', 
                             'Sensor_FL_gal', 'Predicted_FL_Gal', 'Fuel_kg', 'FuelFlow_galph', 'Predicted_FF_galph', 'BusVoltage_vdc', 
                             'TrackX_ft', 'TrackY_ft', 'TrackZ_ft', 'NumSats', 'VisibleSats', 'PDOP', 'PosGood', 'RSSI', 'GSRSSI', 
                             'PilotSample_pct',	'PilotSampleRate', 'UplinkRatio', 'DownlinkRatio', 'APGlobal', 
                             'Wind_south_kts', 'Wind_west_kts', 'Lat_rad', 'Long_rad', 'L_CHT_A', 'GroundSpeed_ftps', 
                             'EngineRunTime_hr', 'IAT_Celcius', 'StaticPressure_Pa', 
                             'P_Bias_radps', 'Q_Bias_radps', 'R_Bias_radps', 'AX_Bias_mps2', 'AY_Bias_mps2', 'AZ_Bias_mps2', 
                             'L_Volts', 'L_Fuel_Press_Pa', 'L_Fuel_Press_PSI', 'L_Fuel_Press_Hg', 'L_aileron_deg', 'R_aileron_deg', 'OAT_Celcius'))
  log <- log %>%
    mutate(year = year_filter,
           month = month_filter, 
           day = day_filter) %>%
    mutate(lat_deg = lat_rad * 180 / pi,
           lon_deg = long_rad * 180 / pi) %>%
    select(tolower(c('Time_sec', 'Year', 'Month', 'Day', 'Hours', 'Minutes', 'Seconds',
                     'Lat_deg', 'Lon_deg', 'Height_ft', 
                     'GroundSpeed_ftps', 'NumSats', 'VisibleSats', 'BaroAlt_ft', 
                     'Roll_deg', 'Pitch_deg', 'AGL_ft', 
                     'Wind_south_kts', 'Wind_west_kts'))) %>%
    sf::st_as_sf(coords = c("lon_deg", "lat_deg"), crs = 4326)
}
